...
class TuringMachineSimulatorHelper {
  public:
     ...
     */
    static bool IsPointInRectangle(const glm::vec2 &point, const ci::Rectf 
        &rectangle);

    /**
     * This method takes in a point, a center of a circle, and the radius of the
     * circle and returns a bool that is true if the point is inside (or on the 
     * border of) the circle and false otherwise
     * 
     * @param point a vec2 representing a point
     ...
     */
    static bool IsPointInCircle(const glm::vec2 &point, const glm::vec2
        &circle_center, double radius);
     ...
     * it returns 0). The size of the tape is returned if no square on the tape 
     * was clicked
     * 
    ...
    static size_t GetIndexOfSquareOfTapeClicked(const glm::vec2 &clicked_point,
        size_t tape_length, const glm::vec2 &tape_upper_corner, const glm::vec2 
        &tape_lower_corner);
    
    /**
     * This method takes in the endpoints of a line and returns a tuple storing
     * the points of a triangle to make the directional arrow for the line. The
     * center point is at index 1 and the other 2 points are at indices 0 and 2.
     * 
     * @param point_a a vec2 representing one endpoint of a line
     * @param point_b a vec2 representing the other endpoint of the line
     * @return a tuple containing the points of the triangle for the arrow, the
     *     center point is located at index 1, with the other 2 points at 
     *     indices 0 and 2
     */
    static std::tuple<glm::vec2, glm::vec2, glm::vec2> GetArrow(const glm::vec2 
        &point_a, const glm::vec2 &point_b);
     ...
     *     diagram
     */
    static void AddDirection(const std::vector<std::string> &add_direction_inputs, 
        std::vector<Direction> &directions, const std::vector<State> &states);
    
    /**
     * This method updates the state that the user is clicking on's position to 
     * be the location that the user clicked
     * 
     * @param clicked_state a State representing the State that the user clicked 
     * @param states a vector of States representing the States in the State
     *     diagram
    ...
    /**
     * This method resets the tape to have 8 squares containing blank characters
     * 
     ...
     * @param blank_character a char representing the turing machine's blank
     *     character
     * @param index_of_character_to_edit a size_t representing the index of the
     *     character to edit
     ...
     *     entered
     * @return a size_t representing the updated index of the character being 
     *     edited
     */
    static size_t UpdateTapeCharacter(std::vector<char> &tape, char 
        blank_character, size_t index_of_character_to_edit, char typed_char,
        int event_code);
     ...
     *     otherwise
     */
    static std::tuple<char, bool> UpdateBlankCharacter(char typed_char, 
        int event_code);
     ...
     * 
     * @param add_arrow_inputs a vector of strings representing the add arrow 
     *     box's inputs
     * @param index_of_input_to_edit a size_t representing the index of the add 
     *     arrow input to edit
     * @param typed_char a character representing the character the user typed
     * @param event_code an int representing the event code of the key that the
     *     user entered
     * @return a size_t representing the the updated index of the add arrow 
     *     input being edited
     */
    static size_t UpdateAddArrowInputs(std::vector<std::string> &add_arrow_inputs, 
        size_t index_of_input_to_edit, char typed_char, int event_code);
};
